...
function xueshan_trigger()
    ...
    create_trigger_t("xueshan_ask1", "^(> )*宝象上下打量着你，嘿嘿淫笑了两声。", "", "job_xueshan")
    create_trigger_t("xueshan_ask2", "^(> )*你向宝象打听有关", "", "xueshan_ask")
    create_trigger_t("xueshan_ask3", "^(> )*这里没有这个人。$", "", "xueshan_nobody")
...
function job_xueshan_ti()
    exe("ask bao xiang about job") --------------雪山ask job 函数
end
function job_xueshan()
    ...
    exe("nick 雪山抢美女任务中")
    flag.idle = nil
    create_timer_s("girlbusy", 0.9, "job_xueshan_ti") ----------增加雪山ask job 计时器
end
function xueshan_ask()
    EnableTriggerGroup("xueshan_ask", false)
    EnableTriggerGroup("xueshan_accept", true)
    EnableTimer("girlbusy", false) ------------关闭雪山ask job 计时器
end
...
function xueshan_fangqi()
    EnableTimer("xsfinishti", false) -------------关闭雪山交任务计时器
    nobusy = 0
...
function xueshan_fangqi_ask()
    EnableTimer("killti", false) -------------关闭雪山kill计时器
    EnableTimer("xsfinishti", false) -------------关闭雪山交任务计时器
    EnableTriggerGroup("xueshan_ask", true)
...
function xueshan_shibai_ask()
    EnableTriggerGroup("xueshan_ask", true)
    EnableTimer("lookti", false) -------------关闭雪山看美女计时器
...
function xueshan_finish()
    ...
    if
        score.party == "姑苏慕容" and need_dzxy == "yes" and string.find(jiangnan_area, locl.area) and hp.pot > 0 and
            hp.food > 50 and
            hp.water > 50
     then
...
function xueshan_finish_ask1()
    create_timer_s("xsfinishti", 0.9, "xueshan_finish_ask_ti") -----------------增加雪山交任务计时器
end
...
function xueshan_look_ti() ----------------增加雪山看美女函数
    exe("look beauty")
end
function xueshan_look()
    ...
    if flag.wait == 0 then
        ...
        job.cnt = 1
        exe("hp;look beauty")
        create_timer_s("lookti", 0.9, "xueshan_look_ti") -------------增加雪山看美女计时器
    end
end
...
function xueshan_kill_act()
    EnableTimer("lookti", false) -------------关闭雪山看美女计时器
    create_timer_s("killti", 0.9, "xueshan_kill_ti") -------------增加雪山kill时器
    exe("kill " .. guard_id)
end
