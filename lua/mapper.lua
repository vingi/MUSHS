...
local rooms = {}
setmetatable(rooms, {__mode = "k"}) --weak table
local last_visited = {}
setmetatable(last_visited, {__mode = "k"}) --weak table
-- other locals
local HALF_ROOM, connectors, half_connectors, arrows
local plan_to_draw={}
local speedwalks={}
local drawn={}
local drawn_coords={} --table need to clear
--setmetatable(plan_to_draw, {__mode = "k"})
--setmetatable(speedwalks, {__mode = "k"})
--setmetatable(drawn, {__mode = "k"})
--setmetatable(drawn_coords, {__mode = "k"})
...
local function draw_special_direction(dir)
   return nil
end
...
local function draw_room (uid, path, x, y)
  ...
  for dir, exit_uid in pairs (room.exits) do
	...
	elseif dir == "nu" or dir=="nd" then
      if not room.exits.n then
        exit_info = connectors.n
        stub_exit_info = half_connectors.n
        arrow = arrows.n
        exit_line_colour = config.EXIT_COLOUR_UP_DOWN.colour
      end -- if
	elseif dir == "su" or dir=="sd" then
      if not room.exits.s then
        exit_info = connectors.s
        stub_exit_info = half_connectors.s
        arrow = arrows.s
        exit_line_colour = config.EXIT_COLOUR_UP_DOWN.colour
      end -- if
	elseif dir == "eu" or dir=="ed" then
      if not room.exits.e then
        exit_info = connectors.e
        stub_exit_info = half_connectors.e
        arrow = arrows.e
        exit_line_colour = config.EXIT_COLOUR_UP_DOWN.colour
      end -- if
	elseif dir == "wu" or dir=="wd" then
      if not room.exits.w then
        exit_info = connectors.w
        stub_exit_info = half_connectors.w
        arrow = arrows.w
        exit_line_colour = config.EXIT_COLOUR_UP_DOWN.colour
      end -- if
    elseif dir~="e" and dir~="w" and dir~="s" and dir~="n" and dir~="ne" and dir~="nw" and dir~="se" and dir~="sw" then
	    --local al=alias.new() --从alias类表中获取特殊路径
		local dx=draw_special_direction(dir)
		--print(dx," dx","->",dir)
		if not room.exits[dx] then
		  exit_info = connectors[dx]
          stub_exit_info = half_connectors[dx]
          arrow = arrows[dx]
          exit_line_colour = config.EXIT_COLOUR_UP_DOWN.colour
	    end
    end -- if down
  ...
  elseif uid==current_room then
      WindowCircleOp (win, miniwin.circle_rectangle, left, top, right, bottom,
                    0, miniwin.pen_null, 0,  -- no pen
                    ColourNameToRGB "yellow", room.fillbrush)  -- brush

      WindowCircleOp (win, miniwin.circle_rectangle, left, top, right, bottom,
                    ColourNameToRGB "red", room.borderpen, room.borderpenwidth,  -- pen
                    -1, miniwin.brush_null)  -- opaque, no brush
  else
    WindowCircleOp (win, miniwin.circle_rectangle, left, top, right, bottom,
                    0, miniwin.pen_null, 0,  -- no pen
  ...
  if room.exits.enter then  -- line at right
...
function draw (uid)
  ...
  draw_text_box (win, FONT_ID,
                 (config.WINDOW.width - WindowTextWidth (win, FONT_ID, room_name, false)) / 2,   -- left
                 3,    -- top
  ...
  if areaname then
    draw_text_box (win, FONT_ID,
                   (config.WINDOW.width - WindowTextWidth (win, FONT_ID, areaname, false)) / 2,   -- left
                   config.WINDOW.height - 3 - font_height,    -- top
                   areaname, false,              -- what to draw, utf8
...
function init (t)
  ...
  start_speedwalk=t.start_speedwalk --我自己快速行走函数
  draw_special_direction=t.draw_special_direction --画特殊路径函数
  ...
  WindowShow (win, true)
  mapper.mapprint (string.format ("MUSHclient mapper installed, version %0.1f", VERSION))
end -- init
...
function mouseup_room (flags, hotspot_id)
  ...
  --print("start_speedwalk")
  --start_speedwalk (speedwalks [uid)--我修改了
  start_speedwalk(uid)
