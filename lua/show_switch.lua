  ...
  local _mousedown_newbie=mousedown_newbie()
   ...
   _G["at_mousedown_ebook"]=_mousedown_ebook
	WindowAddHotspot(win2, "btn_ebook",
	 ...
	 WindowText (win2, "f", switch_name,0,0,140,20,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name2,0,20,140,40,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name3,0,40,140,60,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name4,0,60,140,80,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name5,0,80,140,100,ColourNameToRGB ("gold"), false) -- not Unicode
	  
	 WindowText (win2, "f", switch_name6,0,100,140,120,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name7,0,120,140,140,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name8,0,140,140,160,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name9,0,160,140,180,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name10,0,180,140,200,ColourNameToRGB ("gold"), false) -- not Unicode
	 
	 WindowText (win2, "f", switch_name11,0,200,140,220,ColourNameToRGB ("gold"), false) -- not Unicode
  ...
  return function (flags, hotspot_id)
	...
	if hotspot_id=='btn_smyteam' then
		...
		if flags==32 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,0,140,20,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name,0,0,140,20,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
	end
  ...
  return function (flags, hotspot_id)
	...
	if hotspot_id=='btn_wdj' then
		...
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,40,140,60,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name3,0,40,140,60,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
  ...
  return function (flags, hotspot_id)
	...
	if hotspot_id=='btn_kdummy' then
		...
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,80,140,100,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name5,0,80,140,100,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
  ...
  return function (flags, hotspot_id)
	...
	if hotspot_id=='btn_waitkill' then
		...
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,140,140,160,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name8,0,140,140,160,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
  ...
  return function (flags, hotspot_id)
	...
	if hotspot_id=='btn_dohs' then
		...
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,160,140,180,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name9,0,160,140,180,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
  ...
  return function (flags, hotspot_id)
	...
	if hotspot_id=='btn_ypt_lian' then
		...
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,100,140,120,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name6,0,100,140,120,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
...
function mousedown_taoyuan()
  return function (flags, hotspot_id)
	...
	if hotspot_id=='btn_taoyuan' then
		if intaoyuan==1 then
			intaoyuan=0
			map.rooms["dali/yideng/pubu"].ways["#yuRen"]=nil --桃园县地图加入开关
		else
			intaoyuan=1
			map.rooms["dali/yideng/pubu"].ways["#yuRen"]="dali/yideng/shandong" --桃园县地图加入开关
		end
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,120,140,140,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name7,0,120,140,140,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
  ...
  return function (flags, hotspot_id)
	...
	if hotspot_id=='btn_ebook' then
		...
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,220,140,240,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name12,0,220,140,240,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
...
function mousedown_go_on_smy() --20161117增加变量go_on_smy开关控制 防止系统重启后自动打开颂摩崖
  return function (flags, hotspot_id)
    --print("hotspot_id="..hotspot_id)
	--print("flags="..flags)
	...
	if hotspot_id=='btn_go_on_smy' then
		...
		if flags==16 then
		  ...
		  WindowRectOp (win2, miniwin.rect_fill, 0,200,140,220,ColourNameToRGB("black"))  -- raised, filled, softer, flat 0x909090
		  WindowFont (win2, "f", "新宋体", 10, true, false, false, false)
		  WindowText (win2, "f", switch_name11,0,200,140,220,ColourNameToRGB ("gold"), false) -- not Unicode
		  WindowShow (win2,  true)  -- show it
		end
...
function goll()
	if not getAddr(lostletter_locate) then 
			...
			dest.rooms=getRooms(ll.room,ll.area) --获取玩家所在地的房间

			if table.getn(dest.rooms)==0 then

			   ColourNote ("gold","black","房间无法到达，请重新看信!")

			   return   --获取玩家所在地房间出错，重新看信
